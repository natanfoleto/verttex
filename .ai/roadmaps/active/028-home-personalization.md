...
### Push 1 — Identidade Anônima e Isolamento do Carrinho `[IMPLEMENTADO E RECERTIFICADO LOCALMENTE]`
...
- **Status:** A linha corretiva foi consolidada na `main` local em 2026-08-07, com base no SHA `d09e635`. O conteúdo consolidado com o hardening da baseline passou em `pnpm verify` (372/372 testes). Referência histórica: `PUSH_1J_EVIDENCE.md`.

### Push 2 — Fronteira Canônica do Catálogo e Oferta Real `[NÃO INICIADO — REFAZER DO ZERO]`

- **Status:** O commit histórico `b0ac3d1` foi removido da `main` local por decisão do responsável em 2026-08-07. Ele continua alcançável em `origin/main` até futura sincronização remota, mas não deve ser reaplicado silenciosamente. O Push 2 será planejado e implementado novamente a partir da baseline certificada do Push 1.
...
- **Estado parcial da baseline:** A remoção preventiva de produtos demonstrativos e alegações comerciais fabricadas foi antecipada. Endpoint personalizado e tracking continuam pendentes.
