...
export default function NotificationsCenterPage() {
  ...
  const { data, isLoading } = useQuery<{ unreadCount: number; notifications: NotificationItem[] }>({
    queryKey: ["notifications", unreadOnly],
    queryFn: async () => {
      try {
        const res = await apiClient<any>(`/notifications?unreadOnly=${unreadOnly}`);
        return res.data;
      } catch {
        return {
          unreadCount: 1,
          notifications: [
            {
              ...
              message: "Aviso: O lote CAN-2026-02 do produto 'Queijo Canastra Tradicional' atinge a faixa de aviso de 30 dias para vencimento.",
