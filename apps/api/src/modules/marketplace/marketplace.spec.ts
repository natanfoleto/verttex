...
describe("Marketplace & Carousel Module", () => {
  ...
  afterEach(async () => {
    ...
    if (savedSettings) {
      const { id, createdAt, updatedAt, ...restorable } = savedSettings;
  ...
  describe("Zod Schemas & Security Validations", () => {
    ...
    it("should allow safe internal and external URLs", () => {
      ...
      const externalInput = { title: "Test", linkUrl: "https://example.com/promo" };

      expect(createCarouselBannerSchema.safeParse(internalInput).success).toBe(true);
    ...
    it("should reject invalid outOfStockBehavior enums", () => {
      ...
      expect(updateMarketplaceSettingsSchema.safeParse(validEnum).success).toBe(true);
    });
  });
  ...
  describe("Marketplace Settings (Singleton Service)", () => {
    ...
    it("should project safe public settings", async () => {
      ...
      expect((publicSettings as any).updatedBy).toBeUndefined();
    });
  });
  ...
  describe("Carousel Service (Banners)", () => {
    ...
    it("should update banner text and status", async () => {
      const banner = await carouselService.createBanner({ title: "__TEST__ Banner A" }, "user-1");
      createdBannerIds.push(banner.id);
    ...
    it("should filter out active banners WITHOUT images from public listing", async () => {
      ...
      const testActive = allActive.filter((b) => b.title.startsWith("__TEST__") && b.imageUrl);
    ...
    it("should delete image ONLY and keep banner record", async () => {
      ...
      const afterImageDeleted = await carouselService.deleteBannerImage(banner.id, "user-1");
      expect(afterImageDeleted.imageUrl).toBeNull();
      expect(afterImageDeleted.fileId).toBeNull();

      const bannerStillExists = await prisma.carouselBanner.findUnique({ where: { id: banner.id } });
      expect(bannerStillExists).not.toBeNull();
      expect(bannerStillExists?.title).toBe("__TEST__ Banner para Remover Imagem");
    });

    it("should delete banner record completely", async () => {
      const banner = await carouselService.createBanner({ title: "__TEST__ Banner Excluir" }, "user-1");
      // Não precisa rastrear: o próprio teste chama deleteBanner
      await carouselService.deleteBanner(banner.id);

      const found = await prisma.carouselBanner.findUnique({ where: { id: banner.id } });
      expect(found).toBeNull();
    });

    it("should reorder banners correctly", async () => {
      const b1 = await carouselService.createBanner({ title: "__TEST__ Reorder B1", position: 9990 }, "user-1");
      const b2 = await carouselService.createBanner({ title: "__TEST__ Reorder B2", position: 9991 }, "user-1");
      ...
      const b1Updated = await prisma.carouselBanner.findUnique({ where: { id: b1.id } });
      const b2Updated = await prisma.carouselBanner.findUnique({ where: { id: b2.id } });
