...
describe("CategoriesService Unit & Security Tests", () => {
  it("should correctly normalize category slugs removing special characters and accents", () => {
    expect(normalizeSlug("Queijos & Embutidos Artesanais")).toBe(
      "queijos-embutidos-artesanais",
    );
    expect(normalizeSlug("Doces da Vovó (Serra da Canastra)")).toBe(
      "doces-da-vovo-serra-da-canastra",
    );
  });
  ...
  it("should prevent setting a category as parent of itself", async () => {
    ...
    if (existingCat) {
      await expect(
        categoriesService.updateCategory(existingCat.id, {
