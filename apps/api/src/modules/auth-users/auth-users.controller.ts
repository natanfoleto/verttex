...
export async function logoutController(
  ...
  try {
    ...
    if (token) {
      const decoded = request.server.jwt.decode<{ jti?: string; exp?: number }>(
        token,
      );
      jti = decoded?.jti;
...
export async function listSessionsController(
  ...
  const sessions = await authUsersService.listUserSessions(
    userId,
    currentSessionId,
  );
...
export async function revokeSessionController(
  ...
  const result = await authUsersService.revokeSession(
    userId,
    params.sessionId,
    request,
  );
...
export async function revokeOtherSessionsController(
  ...
  const userId = request.userPayload!.id;
  const currentSessionId = request.userPayload!.sessionId!;
  const result = await authUsersService.revokeOtherSessions(
    userId,
    currentSessionId,
    request,
  );
