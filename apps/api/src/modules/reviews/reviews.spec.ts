  ...
  describe('createReview (Verified Purchase)', () => {
    it('should allow review creation when customer has a DELIVERED order for the product', async () => {
      ...
      const result = await ReviewsService.createReview('cust-1', {
        ...
        rating: 5,
        comment:
          'Excelente queijo canastra, sabor autêntico e maturação perfeita!',
      })
  ...
  describe('createQuestion & answerQuestion', () => {
    it('should submit a question and allow merchant to answer', async () => {
      ...
      expect(q.question).toBe(
        'Este queijo precisa ficar refrigerado durante o transporte?',
      )
      ...
      const answered = await ReviewsService.answerQuestion(
        ...
        {
          answer:
            'Sim! Enviamos em embalagem térmica climatizada com controle sanitário de temperatura.',
        },
      )
