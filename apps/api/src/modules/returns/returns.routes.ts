...
export async function returnsRoutes(app: FastifyInstance) {
  ...
  typedApp.post(
    "/:returnId/quarantine-entry",
    {
      ...
      schema: {
        tags: ["Returns & Quarantine"],
        summary: "Registrar recebimento e entrada em Quarentena Sanitária compulsória",
        security: [{ bearerAuth: [] }],
  ...
  typedApp.post(
    "/:returnId/quarantine-release",
    {
      ...
      schema: {
        tags: ["Returns & Quarantine"],
        summary: "Registrar laudo de inspeção sanitária e liberação/descarte da quarentena",
        security: [{ bearerAuth: [] }],
        params: z.object({ returnId: z.string() }),
