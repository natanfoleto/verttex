...
export async function updateProductController(
  ...
  const product = await ProductsService.updateProduct(params.id, body, userId, request)
...
export async function publishProductController(
  ...
  const params = request.params as { id: string }
  const product = await ProductsService.publishProduct(params.id, userId, request)
...
export async function archiveProductController(
  ...
  const params = request.params as { id: string }
  const result = await ProductsService.archiveProduct(params.id, userId, request)
