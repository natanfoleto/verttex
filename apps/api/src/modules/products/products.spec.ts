...
describe('Products & Catalog Service', () => {
  it('should create a simple product and auto-generate default variation with price', async () => {
    ...
    expect(product.variations?.[0]?.sku).toBe(`MINAS-FRESCAL-${randomSuffix.toUpperCase()}`)
  ...
  it('should publish active product when all readiness conditions are met', async () => {
    const store = await prisma.store.findFirst({ where: { status: 'active', deletedAt: null } })
    const category = await prisma.category.findFirst({ where: { deletedAt: null } })
    const adminUser = await prisma.user.findFirst()
    ...
    const published = await ProductsService.publishProduct(product.id, adminUser.id)
  ...
  it('should archive product via soft-delete', async () => {
    ...
    const archivedInDb = await prisma.product.findUnique({ where: { id: product.id } })
