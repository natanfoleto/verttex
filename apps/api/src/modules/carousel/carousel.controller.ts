...
export class CarouselController {
  ...
  async createBanner(req: FastifyRequest, reply: FastifyReply) {
    const userId = (req.user as any)?.id || (req as any).userPayload?.id || "system";
    const banner = await carouselService.createBanner(req.body as any, userId);
  ...
  async updateBanner(req: FastifyZodRequest, reply: FastifyReply) {
    ...
    const updated = await carouselService.updateBanner(params.id, req.body, userId);
  ...
  async deleteBannerImage(req: FastifyZodRequest, reply: FastifyReply) {
    ...
    return reply.send({ success: true, data: updated, message: "Imagem do banner removida com sucesso." });
  }
  ...
  async deleteBanner(req: FastifyZodRequest, reply: FastifyReply) {
    ...
    return reply.send({ success: true, message: "Banner removido com sucesso." });
  }
  ...
  async reorderBanners(req: FastifyRequest, reply: FastifyReply) {
    ...
    return reply.send({ success: true, message: "Banners reordenados com sucesso." });
  }
}
