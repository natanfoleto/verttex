...
import { validateCNPJ, validateCPF, validateCPForCNPJ } from "../../shared/utils/cpf-cnpj";
  ...
  describe("Customer Addresses Service Logic", () => {
    ...
    it("should list customer addresses ordered by default first", async () => {
      vi.mocked(prisma.customerAddress.findMany).mockResolvedValue([
        { id: "addr-1", ...sampleAddressInput, isDefault: true, customerId: mockCustomerId, createdAt: new Date(), updatedAt: new Date() },
      ]);

      const result = await CustomerAddressesService.listCustomerAddresses(mockCustomerId);
    ...
    it("should throw NOT_FOUND error when accessing non-existent or unowned address", async () => {
      ...
      await expect(
        CustomerAddressesService.getCustomerAddressDetails(mockCustomerId, "invalid-id"),
      ).rejects.toThrow("Endereço não encontrado ou não pertence a este cliente");
    });
    ...
    it("should delete address and re-assign default address to remaining address if default address was deleted", async () => {
      ...
      await CustomerAddressesService.deleteCustomerAddress(mockCustomerId, mockAddressId);
