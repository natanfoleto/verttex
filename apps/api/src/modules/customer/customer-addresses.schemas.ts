...
export const createAddressBodySchema = z
  .object({
    ...
    recipient: z.string().min(2, "Nome do destinatário deve ter no mínimo 2 caracteres"),
    ...
    state: z.string().length(2, "Estado/UF deve conter 2 letras (ex: RS)").transform((val) => val.toUpperCase()),
...
export const updateCustomerProfileBodySchema = z
  .object({
    name: z.string().min(2, "Nome deve conter no mínimo 2 caracteres").optional(),
    phone: z.string().optional(),
    cpfCnpj: z
      .string()
