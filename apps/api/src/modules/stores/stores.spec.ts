...
describe('Stores Management & Reserved Slugs Unit Tests', () => {
  it('should correctly normalize store name into a URL-friendly slug', () => {
    expect(normalizeSlug('Queijaria Alvorada da Serra!')).toBe(
      'queijaria-alvorada-da-serra',
    )
    expect(normalizeSlug('Vinícola & Adega Rossi (RS)')).toBe(
      'vinicola-adega-rossi-rs',
    )
    expect(normalizeSlug('  Doces & Geleias - Colônia  ')).toBe(
      'doces-geleias-colonia',
    )
  })
