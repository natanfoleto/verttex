...
export class CartService {
  ...
  static async getOrCreateCart(owner: CartOwner) {
    if (!owner.customerId && !owner.sessionId) {
      throw new AppError(
        'VALIDATION_ERROR',
        'Proprietário do carrinho não identificado',
        400,
      )
    }
  ...
  static async getCartSummary(owner: CartOwner) {
    ...
    const storesMap = new Map<string, CartStoreGroup>()
    let rawSubtotal = 0
    ...
    let totalDiscount = 0
    const appliedCoupons: CartAppliedCoupon[] = []
    ...
    for (const cartCoupon of fullCart.coupons) {
      ...
      if (
        ...
        eligibleSubtotal < Number(coupon.minOrderValue)
      ) {
        continue
      }
    ...
    const totalItemsCount = fullCart.items.reduce(
  ...
  static async addItem(
    ...
    if (
      ...
      !variation.product.isPublished
    ) {
      throw new AppError(
        'NOT_FOUND',
        'Variação do produto indisponível ou inativa',
        404,
      )
    }
    ...
    const totalAvailableStock = stockItems.reduce(
      (
        acc: number,
        item: { physicalQuantity: number; reservedQuantity: number },
  ...
  static async updateItemQuantity(
    ...
    const totalAvailableStock = stockItems.reduce(
      (
        acc: number,
        s: { physicalQuantity: number; reservedQuantity: number },
