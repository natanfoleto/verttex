...
function setupFacetMocks() {
  ...
  mockPrisma(prisma.category.findMany).mockResolvedValue(
  ...
  mockPrisma(prisma.productVariation.findMany).mockImplementation(async () => {
    ...
    return allVariations as unknown as Awaited<
...
---------------------------------------------------`)
  ...
  it('28. Self-excluding facets (Contagens disjuntivas corretas por faceta)', async () => {
    ...
    const floradaFacet = res.availableFilters.find(
      (f) => f.key === 'Florada' || f.key === 'attr_florada',
    )

    expect(floradaFacet).toBeDefined()
    const silvestreOpt = floradaFacet?.options.find(
      (o) => o.value === 'Silvestre',
    )
    const eucaliptoOpt = floradaFacet?.options.find(
      (o) => o.value === 'Eucalipto',
    )
  ...
  it('29. COUNT DISTINCT Product: Produto com múltiplas variantes que atendem a faceta conta 1 produto', async () => {
    ...
    const madeiraFacet = res.availableFilters.find(
      (f) => f.key === 'Madeira' || f.key === 'attr_madeira',
    )

    expect(madeiraFacet).toBeDefined()
    const amburanaOpt = madeiraFacet?.options.find(
      (o) => o.value === 'Amburana',
    )
