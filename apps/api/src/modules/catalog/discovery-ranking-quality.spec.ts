...
describe("Discovery Quality — Ranking & Relevância de Pesquisa", () => {
  ...
  it("12. Ranking por pesos de campos: Título > Contexto > Atributos > Descrição", async () => {
    ...
    const mockFullProducts = mockSearchDocs.map((d) => ({
      ...
      store: { id: "st-1", name: "Store", slug: "store", isPublished: true, status: "active", deletedAt: null },
      images: [{ url: "https://example.com/img.jpg" }],
      medias: [{ isMain: true, file: { objectKey: "img.jpg" } }],
      variations: [{ id: `v-${d.productId}`, price: d.price, values: [], stockItems: [{ quantity: 10 }] }],
    }));
    ...
    mockPrisma(prisma.product.findMany).mockImplementation(
      async (args?: Parameters<typeof prisma.product.findMany>[0]) => {
        ...
        if (args?.where?.OR) {
          return [] as unknown as Awaited<ReturnType<typeof prisma.product.findMany>>;
        }
        return mockFullProducts as unknown as Awaited<ReturnType<typeof prisma.product.findMany>>;
      }
    );

    const res = await PublicDiscoveryService.discover({ search: term, page: 1, perPage: 50 });
    const receivedOrder = res.products.map((p) => p.id);

    const expectedOrder = ["prod-title", "prod-context", "prod-attr", "prod-desc"];
  ...
  it("13. Ranking multi-campo: Título + Contexto vence produto apenas com Título", async () => {
    const mockSearchDocs = [
      ...
      },
      {
        ...
        descriptionNormalized: "",
        searchTextNormalized: "mel silvestre orgânico mel artesanal serra mel de abelha",
        price: 30,
        inStock: true,
      },
    ];
      ...
      async (args?: Parameters<typeof prisma.productVariation.findMany>[0]) => {
        if (args?.where?.OR && Array.isArray(args.where.OR)) {
          ...
          if (skuTerm === "MEL-SILV-500G") {
            return [{ productId: "prod-sku-match" }] as unknown as Awaited<ReturnType<typeof prisma.productVariation.findMany>>;
          }
        }
