...
function setupGoldenDatasetMocks() {
  ...
  const searchDocs = activeProducts.map((p) => {
    ...
    const contextNormalized = normalizeSearchText(
      `${p.categoryName} ${p.brandName || ''} ${p.storeName}`,
    )
    const attributesNormalized = p.attributes
      .map((a) => normalizeSearchText(a.value))
      .join(' ')
      ...
      stockItems: [
        {
          ...
          lot: { expiresAt: new Date('2028-01-01'), status: 'AVAILABLE' },
        },
      ],
    })),
  }))
  ...
  mockPrisma(prisma.productSearchDocument.findMany).mockImplementation(
    async (
      ...
      const catSlug = (args?.where as { categorySlug?: string } | undefined)
        ?.categorySlug
      const bSlug = (args?.where as { brandSlug?: string } | undefined)
        ?.brandSlug
      const sSlug = (args?.where as { storeSlug?: string } | undefined)
        ?.storeSlug
        ...
        const matched = fullProducts.flatMap((p) =>
          p.variations
            .filter(
              (v) =>
                (skuTerm &&
                  (v.sku?.toLowerCase() === skuTerm.toLowerCase() ||
                    p.sku?.toLowerCase() === skuTerm.toLowerCase())) ||
                (barcodeTerm &&
                  (v.barcode?.toLowerCase() === barcodeTerm.toLowerCase() ||
                    p.barcode?.toLowerCase() === barcodeTerm.toLowerCase())),
            )
            .map(() => ({ productId: p.id })),
        )
        return matched as unknown as Awaited<
      ...
      if (args?.where?.OR && Array.isArray(args.where.OR)) {
        ...
        if (skuEq || barcodeEq) {
          result = result.filter(
            (p) =>
              (skuEq &&
                (p.sku?.toLowerCase() === skuEq.toLowerCase() ||
                  p.variations?.some(
                    (v) => v.sku?.toLowerCase() === skuEq.toLowerCase(),
                  ))) ||
              (barcodeEq &&
                (p.barcode?.toLowerCase() === barcodeEq.toLowerCase() ||
                  p.variations?.some(
                    (v) => v.barcode?.toLowerCase() === barcodeEq.toLowerCase(),
                  ))),
          )
        } else {
          const firstTermOr = args.where?.OR?.[0] as
          ...
          if (tokens.length > 0) {
            result = result.filter((p) => {
              ...
              return doc
                ? tokens.every((t) => doc.searchTextNormalized.includes(t))
                : false
            })
          }
        }
...
function assertExpectedSearchResult(
  ...
  const unexpectedForbidden = receivedIds.filter((id) =>
...
describe('Discovery Quality — Busca Simples & Termos', () => {
  ...
  it("6. Case Insensitive — 'MEL', 'Mel' e 'mel' devem retornar os mesmos produtos", async () => {
    const resLower = await PublicDiscoveryService.discover({
      search: 'mel',
      page: 1,
      perPage: 50,
    })
    const resUpper = await PublicDiscoveryService.discover({
      search: 'MEL',
      page: 1,
      perPage: 50,
    })
    const resMixed = await PublicDiscoveryService.discover({
      search: 'Mel',
      page: 1,
      perPage: 50,
    })
  ...
  it("7. Normalização de acentos — 'cachaça' vs 'cachaca'", async () => {
    const resAccented = await PublicDiscoveryService.discover({
      search: 'cachaça',
      page: 1,
      perPage: 50,
    })
    const resNormalized = await PublicDiscoveryService.discover({
      search: 'cachaca',
      page: 1,
      perPage: 50,
    })
  ...
  it("8. Espaços duplos e extras — '  mel   silvestre  '", async () => {
    const resNormal = await PublicDiscoveryService.discover({
      search: 'mel silvestre',
      page: 1,
      perPage: 50,
    })
    const resSpaces = await PublicDiscoveryService.discover({
      search: '  mel   silvestre  ',
      page: 1,
      perPage: 50,
    })

    expect(resSpaces.products.map((p) => p.id)).toEqual(
      resNormal.products.map((p) => p.id),
    )
  })
  ...
  it("11. Ordem dos termos — 'mel silvestre' vs 'silvestre mel'", async () => {
    const res1 = await PublicDiscoveryService.discover({
      search: 'mel silvestre',
      page: 1,
      perPage: 50,
    })
    const res2 = await PublicDiscoveryService.discover({
      search: 'silvestre mel',
      page: 1,
      perPage: 50,
    })
  ...
  it('24. Produto com múltiplas variantes — deve retornar 1 único produto e não N variantes', async () => {
    ...
    const multiVarProducts = res.products.filter(
      (p) => p.id === 'prod-golden-multi-var',
    )
    expect(multiVarProducts.length).toBe(1)
  })

  it("25. Query vazia ou com apenas espaços no fluxo público (/busca, /busca?q=, search: '   ')", async () => {
    const resEmpty = await PublicDiscoveryService.discover({
      search: '',
      page: 1,
      perPage: 50,
    })
    ...
    const resUndefined = await PublicDiscoveryService.discover({
      page: 1,
      perPage: 50,
    })
    ...
    expect(resSpaces.products.map((p) => p.id)).toEqual(
      resEmpty.products.map((p) => p.id),
    )
    expect(resUndefined.products.map((p) => p.id)).toEqual(
      resEmpty.products.map((p) => p.id),
    )
  })
})
