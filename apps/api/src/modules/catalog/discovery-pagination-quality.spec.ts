...
function setupPaginationMocks() {
  ...
  mockPrisma(prisma.product.findMany).mockImplementation(
    async (args?: Parameters<typeof prisma.product.findMany>[0]) => {
      ...
      const catIdIn = (
        args?.where as { categoryId?: { in?: string[] } } | undefined
      )?.categoryId?.in
      const idIn = (args?.where as { id?: { in?: string[] } } | undefined)?.id
        ?.in
      const isPub = (args?.where as { isPublished?: boolean } | undefined)
        ?.isPublished
      const delAt = (args?.where as { deletedAt?: null | Date } | undefined)
        ?.deletedAt
...
describe('Discovery Quality — Paginação & Contextos de Rota', () => {
  ...
  it('45. Ranking + Paginação — Ranking global é calculated ANTES da fatia de página', async () => {
    ...
    const all = await PublicDiscoveryService.discover({
      search: 'mel',
      page: 1,
      perPage: 100,
    })
  ...
  it('46. Paginação Estável — Execuções repetidas retornam a mesma sequência exata', async () => {
    const run1 = await PublicDiscoveryService.discover({
      search: 'cachaca',
      page: 1,
      perPage: 3,
    })
    const run2 = await PublicDiscoveryService.discover({
      search: 'cachaca',
      page: 1,
      perPage: 3,
    })

    expect(run1.products.map((p) => p.id)).toEqual(
      run2.products.map((p) => p.id),
    )
  })
