...
describe('Discovery Quality — Matriz Comercial de Validação (Golden Dataset)', () => {
  ...
  SEED_MATRIX_TEST_CASES.forEach((tc) => {
    it(`[${tc.id}] ${tc.description}`, async () => {
      ...
      const forbiddenPresent = (tc.forbiddenIds || []).filter((id) =>
...
---------------------------------------------------`)
      ...
      if (tc.forbiddenIds && tc.forbiddenIds.length > 0) {
        ...
        const extraProducts = receivedIds.filter(
          (id) =>
            !tc.expectedIds.includes(id) && !tc.forbiddenIds?.includes(id),
        )
