...
function setupGuardsMocks(
  outOfStockBehavior:
    'show_badge' | 'hide_product' | 'move_to_end' = 'show_badge',
) {
  const searchDocs = GOLDEN_DATASET_PRODUCTS.map((p) => {
    ...
    return {
      ...
      inStock:
        p.stockTotal > 0 && !p.hasExpiredStockOnly && !p.hasQuarantineStockOnly,
  ...
  const fullProducts = GOLDEN_DATASET_PRODUCTS.map((p) => ({
    ...
    store: {
      ...
      name: p.storeName,
      slug: p.storeSlug,
      isPublished: p.storeIsPublished,
      status: p.storeIsPublished ? 'active' : 'inactive',
      deletedAt: null,
    },
    images: [],
      ...
      stockItems: [
        {
          ...
          lot: {
            expiresAt: p.hasExpiredStockOnly
              ? new Date('2020-01-01')
              : new Date('2028-01-01'),
  ...
  mockPrisma(prisma.stockItem.findMany).mockImplementation(
    async (args?: Parameters<typeof prisma.stockItem.findMany>[0]) => {
      ...
      return varIds.map((vId) => {
        const parentProd = fullProducts.find((p) =>
          p.variations.some((v) => v.id === vId),
        )
        const hasExpired = parentProd?.id === 'prod-golden-expired-stock'
        const isQuarantine = parentProd?.id === 'prod-golden-quarantine-stock'
        const isShortShelfLife =
          parentProd?.id === 'prod-golden-shelflife-stock'
        ...
        return {
          ...
          variationId: vId,
          physicalQuantity:
            parentProd?.id === 'prod-golden-out-of-stock' ? 0 : 100,
          reservedQuantity: 0,
      ...
      if (w) {
        ...
        if (w.variations?.some?.promotionalPrice) {
          result = result.filter(
            (p) => p.promotionalPrice && p.promotionalPrice < p.price,
          )
        }
        const storeWhere = w.store as
          { status?: string; isPublished?: boolean } | undefined
          ...
          if (skuEq || barcodeEq) {
            result = result.filter(
              (p: Record<string, unknown>) =>
                (skuEq &&
                  ((p.sku as string)?.toLowerCase() === skuEq.toLowerCase() ||
                    (p.variations as Array<{ sku?: string }>)?.some(
                      (v) => v.sku?.toLowerCase() === skuEq.toLowerCase(),
                    ))) ||
                (barcodeEq &&
                  ((p.barcode as string)?.toLowerCase() ===
                    barcodeEq.toLowerCase() ||
                    (p.variations as Array<{ barcode?: string }>)?.some(
                      (v) =>
                        v.barcode?.toLowerCase() === barcodeEq.toLowerCase(),
                    ))),
            )
          } else {
            const firstTermOr = w.OR[0] as
...
describe('Discovery Quality — Guards de Negócio, Preço, Ofertas & Estoque', () => {
  ...
  it('31. Ordenação Explícita por Preço (price_asc e price_desc)', async () => {
    const resAsc = await PublicDiscoveryService.discover({
      sort: 'price_asc',
      page: 1,
      perPage: 50,
    })
    ...
    const resDesc = await PublicDiscoveryService.discover({
      sort: 'price_desc',
      page: 1,
      perPage: 50,
    })
    const pricesDesc = resDesc.products.map(
      (p) => p.promotionalPrice || p.price,
    )
  ...
  it('34. GUARD: Produto NÃO publicado (isPublished: false) NUNCA deve aparecer', async () => {
    ...
    if (ids.includes('prod-golden-unpub')) {
      console.error(
        "FALHA DE GUARD: Produto não publicado 'prod-golden-unpub' apareceu na busca!",
      )
    }
  ...
  it('35. GUARD: Produto Arquivado/Deletado (deletedAt != null) NUNCA deve aparecer', async () => {
    ...
    if (ids.includes('prod-golden-deleted')) {
      console.error(
        "FALHA DE GUARD: Produto deletado 'prod-golden-deleted' apareceu na busca!",
      )
    }
  ...
  it('36. GUARD: Loja/Produtor Inativo (storeIsPublished: false) NUNCA deve aparecer', async () => {
    ...
    if (ids.includes('prod-golden-inactive-store')) {
      console.error(
        "FALHA DE GUARD: Produto de loja inativa 'prod-golden-inactive-store' apareceu na busca!",
      )
    }
  ...
  it('37. GUARD: Produto com apenas lote em quarentena deve ser tratado como indisponível', async () => {
    ...
    const prod = res.products.find(
      (p) => p.id === 'prod-golden-quarantine-stock',
    )
  ...
  it('38. GUARD: Produto com apenas lote abaixo da shelf-life mínima (< 15 dias) deve ser tratado como indisponível', async () => {
    ...
    const prod = res.products.find(
      (p) => p.id === 'prod-golden-shelflife-stock',
    )
  ...
  it('40. outOfStockBehavior = show_badge mantém produto sem estoque na lista mas com isAvailable: false', async () => {
    ...
    const res = await PublicDiscoveryService.discover({
      search: 'Doce de Leite Tradicional Esgotado',
      page: 1,
      perPage: 50,
    })
