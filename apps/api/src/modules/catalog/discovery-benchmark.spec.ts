  ...
  volumeTestCases.forEach((volume) => {
    describe(`Benchmark com catálogo de ${volume.toLocaleString("pt-BR")} produtos`, () => {
      it(`1. Busca simples ('mel') com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Busca simples ('mel'): ${durationMs.toFixed(2)}ms (${result.pagination.total} candidatos)`);
      ...
      it(`2. Busca multi-termo ('mel silvestre') com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Busca multi-termo ('mel silvestre'): ${durationMs.toFixed(2)}ms`);
      ...
      it(`3. Busca por contexto ('Boa Esperança') com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Busca por contexto ('Boa Esperança'): ${durationMs.toFixed(2)}ms`);
      ...
      it(`4. Busca por atributo ('amburana') com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Busca por atributo ('amburana'): ${durationMs.toFixed(2)}ms`);
      ...
      it(`5. Termo inexistente ('xyz-inexistente') com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Termo inexistente: ${durationMs.toFixed(2)}ms`);
      ...
      it(`6. Paginação profunda (page=5) com ${volume} produtos`, async () => {
        ...
        console.log(`[Benchmark ${volume} prods] Paginação (page=5): ${durationMs.toFixed(2)}ms`);
      ...
      it(`7. Busca + Filtros/Facetas (mel + minPrice=20 + categoria=mel) com ${volume} produtos`, async () => {
        ...
        const matchingProds = products.filter(
          (p) => p.categoryId === "cat-mel" && melCandidates.some((c) => c.productId === p.id)
        );
        vi.mocked(prisma.product.findMany).mockResolvedValue(matchingProds as any);
        vi.mocked(prisma.stockItem.findMany).mockResolvedValue([]);
        ...
        console.log(`[Benchmark ${volume} prods] Busca + Filtros + Facetas: ${durationMs.toFixed(2)}ms`);
