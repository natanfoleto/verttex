    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-stone-500">
          Gerencie suas informações cadastrais, endereços e segurança na
          VERTTEX.
        </p>
      </div>
      ...
      <div className="border-b border-stone-200">
        <nav
          className="-mb-px flex space-x-8 text-xs font-semibold"
        ...
        >
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`h-4 w-4 ${tab.isActive ? 'text-emerald-700' : 'text-stone-400'}`}
                />
                <span>{tab.label}</span>
              </Link>
            )
