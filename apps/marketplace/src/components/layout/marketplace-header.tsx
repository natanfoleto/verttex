...
export function MarketplaceHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-white font-sans text-stone-900 antialiased shadow-2xs">
      ...
      <div className="bg-stone-900 px-4 py-1.5 text-xs font-medium text-stone-200">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <span className="truncate">
            🌾 Conectando você aos melhores produtores artesanais da nossa
            região
          </span>
          <div className="hidden items-center space-x-4 text-[11px] text-stone-400 sm:flex">
            <span>Serra Gaúcha — RS</span>
            <span>•</span>
            <span className="font-semibold text-emerald-400">
              Entrega Garantida Direto do Produtor
            </span>
          </div>
        </div>
      </div>

      {/* Tier 2: Main Header Row (Logo, Search & Auth) */}
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-6 px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full rounded-lg border border-stone-200 bg-stone-50 py-2.5 pr-10 pl-10 text-xs text-stone-900 placeholder-stone-400 transition-colors focus:border-emerald-600 focus:bg-white focus:outline-none"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        {/* User Actions Right */}
        <div className="hidden items-center space-x-4 text-xs font-semibold md:flex">
          {/* Wishlist Link */}
          <Link
            href="/produtos"
            className="flex items-center space-x-1.5 rounded-lg border border-stone-200/80 bg-white px-3 py-2 text-stone-700 transition-colors hover:border-emerald-300 hover:text-emerald-800"
            title="Favoritos"
          >
          ...
          {customer ? (
            <div className="flex items-center space-x-2">
              <Link
                href="/perfil"
                className="flex items-center space-x-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3.5 py-2 text-emerald-900 transition-colors hover:bg-emerald-100"
              >
              ...
              <button
                onClick={() => logout()}
                className="cursor-pointer rounded-lg border border-stone-200 p-2 text-stone-400 transition-colors hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600"
                title="Sair da conta"
              >
            ...
            <div className="flex items-center space-x-2">
              <Link
                href="/login"
                className="flex items-center space-x-1 rounded-lg border border-stone-200 bg-white px-3.5 py-2 font-semibold text-stone-700 transition-colors hover:bg-stone-50"
              >
              ...
              <Link
                href="/cadastro"
                className="flex items-center space-x-1 rounded-lg bg-emerald-800 px-4 py-2 font-semibold text-white shadow-xs transition-colors hover:bg-emerald-700"
              >
      ...
      {/* Tier 3: Secondary Category & Navigation Sub-Header Bar (Hover Submenus) */}
      <nav className="hidden border-t border-stone-200/80 bg-stone-50/90 text-xs font-semibold text-stone-800 md:block">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {/* Special Promotion Link Right */}
          <div className="flex items-center space-x-3 py-1">
            <Link
              href="/produtos"
              className="inline-flex items-center space-x-1 rounded-md bg-amber-100 px-2.5 py-1 text-[11px] font-bold text-amber-900 transition-colors hover:bg-amber-200"
            >
              <RiPriceTag3Line className="h-3.5 w-3.5 text-amber-700" />
              <span>Seleção Regional</span>
            </Link>
          </div>
        </div>
      </nav>
            ...
            {customer ? (
              ...
              <div className="grid grid-cols-2 gap-2 pt-2">
                <Link
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center rounded-lg border border-stone-200 bg-white py-2.5 text-xs font-semibold text-stone-700"
                >
                  Entrar
                </Link>
                <Link
                  href="/cadastro"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center rounded-lg bg-emerald-800 py-2.5 text-xs font-semibold text-white shadow-xs"
                >
                  Criar Conta
                </Link>
              </div>
